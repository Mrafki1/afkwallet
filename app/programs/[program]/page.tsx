...
export async function generateStaticParams() {
  ...
  const programs = [...new Set(cards.map(c => c.program))];
  return programs.map(program => ({ program: slugify(program) }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ program: string }> }): Promise<Metadata> {
  ...
  const cards = await getCards();
  const programName = nameFromSlug(programSlug, [...new Set(cards.map(c => c.program))]);
  if (!programName) return {};
  const programCards = cards.filter(c => c.program === programName);
  return {
    title: `${programName} Credit Cards Canada 2026 | PointsBinder`,
    description: `Compare all ${programCards.length} Canadian credit cards that earn ${programName} points. Welcome bonuses, annual fees, and best rebate portal offers.`,
    alternates: { canonical: `/programs/${programSlug}` },
    keywords: [`${programName} credit card`, `${programName} Canada`, "points card Canada", "welcome bonus"],
  };
}
...
export default async function ProgramPage({ params }: { params: Promise<{ program: string }> }) {
  ...
  const programCards = cards.filter(c => c.program === programName).sort((a, b) => {
  ...
  const jsonLd = [
    {
      ...
      "itemListElement": [
        ...
        { "@type": "ListItem", "position": 3, "name": programName, "item": `${siteUrl}/programs/${programSlug}` },
      ],
    },
    {
      "@context": "https://schema.org",
      "@type": "ItemList",
      "name": `${programName} Credit Cards`,
      "description": `Canadian credit cards that earn ${programName} points`,
      "numberOfItems": programCards.length,
      "itemListElement": programCards.map((card, i) => ({
        "@type": "ListItem",
        "position": i + 1,
  ...
  // Transfer partners info (shown if any card has them)
  const allPartners = [...new Set(programCards.flatMap(c => c.transferPartners ?? []))];
      ...
      <div style={{ background: "#0f172a" }}>
        <div className="max-w-5xl mx-auto px-6 py-14">
          <nav className="flex items-center gap-2 text-xs mb-4" style={{ color: "#64748b" }}>
            <Link href="/programs" className="hover:text-blue-400 transition-colors">All Programs</Link>
            <span>/</span>
            <span style={{ color: "#94a3b8" }}>{programName}</span>
          </nav>
          <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#60a5fa" }}>Rewards Program</p>
          <h1 className="text-4xl font-bold mb-3" style={{ color: "#ffffff", letterSpacing: "-0.03em" }}>
            {programName} Credit Cards
          </h1>
          <p className="text-base" style={{ color: "#94a3b8" }}>
            {programCards.length} card{programCards.length !== 1 ? "s" : ""} earning {programName} in Canada
          </p>

          {allPartners.length > 0 && (
            <div className="mt-6 flex flex-wrap gap-2">
              <span className="text-xs font-semibold" style={{ color: "#60a5fa" }}>Transfer partners:</span>
              {allPartners.slice(0, 8).map(p => (
                <span key={p} className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ background: "#1e293b", color: "#94a3b8" }}>
                  {p}
                </span>
              ))}
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="p-5 flex flex-col gap-3 flex-1">
                  <div>
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-0.5">{card.issuer}</p>
